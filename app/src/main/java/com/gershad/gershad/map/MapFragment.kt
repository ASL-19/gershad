...
import com.gershad.gershad.dialogs.GershadOneButtonAlert
import com.gershad.gershad.dialogs.GershadTwoButtonsAlert
...
class MapFragment : Fragment(), OnMapReadyCallback, MapContract.View, MapContract.Adapter, Injects<Module> {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        autocompleteFragment = place_autocomplete_searchbar as AutocompleteSupportFragment
        autocompleteFragment.activity!!.findViewById<View>(R.id.places_autocomplete_search_button).visibility = GONE
        autocompleteFragment.setHint(getString(R.string.search_location_hint))
        autocompleteFragment.view?.findViewById<EditText>(R.id.places_autocomplete_search_input)?.textSize = 15.0f
    ...
    private fun updateReportBottomSheet(context: Context, reportGroup: ReportGroup) {
        ...
        report_details_list.layoutManager = androidx.recyclerview.widget.LinearLayoutManager(context)
        ...
        if (reportGroup.isPermanent) {
            report_type.setText(R.string.hot_spot)
        } else {
            report_type.setText(when(reportGroup.displayType) {
                ReportType.GASHT -> R.string.police
                ReportType.STOP -> R.string.stop
                ReportType.VAN -> R.string.van
                else -> R.string.unspecified
            })
        }

        report_type_icon.setBackground(if(reportGroup.isPermanent){null} else {ContextCompat.getDrawable(context, R.drawable.circle_background_yellow)})
        report_type_icon.setImageDrawable(ContextCompat.getDrawable(context,
            if (reportGroup.isPermanent) {R.drawable.ic_big_hot_spot} else {getIconFor(reportGroup.displayType) }))
        ...
        when (myExistingReport) {
            null -> {
                ...
                button_verify.visibility = if(reportGroup.isPermanent) {View.GONE} else {View.VISIBLE}
                ...
                button_delete.visibility = if(myExistingReport.isPermanent) {View.GONE} else {View.VISIBLE}
                ...
                button_delete.setOnClickListener {
                    reportSheetBehavior.state = BottomSheetBehavior.STATE_COLLAPSED
                    GershadTwoButtonsAlert.newInstance(getString(R.string.delete_warning), null, null, getString(R.string.delete),
                        OnClickListener {
    ...
    override fun showErrorView(e: Throwable) {
        ...
        if (e is HttpException && e.code() == 429) {
            GershadOneButtonAlert.newInstance(getString(R.string.network_error_dialog_title), getString(R.string.report_delay_error_message),
                getString(R.string.network_error_dialog_button_caption)).show(getFragmentManager(), "1")
        } else {
            GershadOneButtonAlert.newInstance(getString(R.string.operation_failed_error_dialog_title), getString(R.string.operation_failed_error_dialog_description),
                getString(R.string.close)).show(getFragmentManager(), "1")
        }
