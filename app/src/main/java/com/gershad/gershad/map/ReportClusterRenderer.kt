...
class ReportClusterRenderer(val context: Context, map: GoogleMap, clusterManager: ClusterManager<ReportGroupClusterItem>)
    ...
    private val gashtIcons by lazy { getDrawableArray(ReportType.GASHT, false) }
    private val permanentGashtIcons by lazy { getDrawableArray(ReportType.GASHT, true) }
    private val vanIcons by lazy { getDrawableArray(ReportType.VAN, false) }
    private val permanentVanIcons by lazy { getDrawableArray(ReportType.VAN, true) }
    private val stopIcons by lazy { getDrawableArray(ReportType.STOP, false) }
    private val permanentStopIcons by lazy { getDrawableArray(ReportType.STOP, true) }
    private val unspecifiedIcons by lazy { getDrawableArray(ReportType.UNSPECIFIED, false) }
    ...
    override fun onBeforeClusterItemRendered(item: ReportGroupClusterItem?, markerOptions: MarkerOptions?) {
        ...
        val iconId: Int = getClusterIcon(item!!.displayType, item.reportCount, item.isPermanent)
     ...
     */
    private fun getClusterIcon(type: ReportType, count: Int, isPermanent: Boolean) : Int {
        val iconIds = when(type) {
            ReportType.GASHT -> {if(isPermanent) {permanentGashtIcons} else {gashtIcons}}
            ReportType.VAN -> {if(isPermanent) {permanentVanIcons} else {vanIcons}}
            ReportType.STOP -> {if(isPermanent) {permanentStopIcons} else {stopIcons}}
     ...
     */
    private fun getDrawableArray(type: ReportType, isPermanent: Boolean): IntArray {
        val arr: TypedArray = when(type) {
            ReportType.VAN -> {
                context.resources.obtainTypedArray(if(isPermanent) {R.array.array_permanent_van_icons} else {R.array.array_van_icons})
            }
            ReportType.GASHT -> {
                context.resources.obtainTypedArray(if(isPermanent) {R.array.array_permanent_gasht_icons} else {R.array.array_gasht_icons})
            }
            ReportType.STOP -> {
                context.resources.obtainTypedArray(if(isPermanent) {R.array.array_permanent_stop_icons} else {R.array.array_stop_icons})
            }
